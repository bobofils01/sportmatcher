...
class ShowSessionFragment : Fragment() {
    ...
    private fun initParticipantsList(){
        viewModel.getSessionParticipants(session).observe(requireActivity(), Observer { playersList ->
            val namesParticipants = playersList.map { t -> t.firstName + " " + t.lastName }
            val adapter = ArrayAdapter(requireContext(), android.R.layout.simple_list_item_1, namesParticipants)
            added_players_recycle_view.adapter = adapter

            if(session.maxNbPlayers == -1 ){
                session_max_players.text = "${namesParticipants.size}/_"
            }else if(namesParticipants.size >= session.maxNbPlayers!!){
                session_max_players.text = "${namesParticipants.size}/${session.maxNbPlayers!!}"
    ...
    private fun initJoinButton(){
        ...
        btn_join_session.setOnClickListener{
            ServiceProvider.joinSessionUseCase.execute(
